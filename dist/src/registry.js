...
function find(library, factory) {
    const found = FACTORIES[`${library}/${factory}`];
    if (found === undefined) {
        throw new BimmError(`unknown factory: ${library}/${factory}\n  catalogue: ${shown()}`);
    }
    return found;
}
/** 이 팩토리가 받는 인자 규격. 매니페스트를 쓰는 쪽이 물어볼 수 있어야 한다. */
export function factorySpec(library, factory) {
 ...
 */
export function createModel(library, factory, args = {}) {
