 ...
 */
export function checkArgs(factory, spec, given) {
    const known = Object.keys(spec);
    const extra = Object.keys(given).filter((k) => !known.includes(k));
    if (extra.length > 0) {
        throw new BimmError(`${factory} 가 모르는 인자입니다: ${extra.sort().join(", ")}\n` +
    ...
    for (const key of known) {
        ...
        if (value === undefined) {
            throw new BimmError(`${factory} 에 ${key} 가 없습니다 — 이 인자는 기본값이 없습니다.\n` +
