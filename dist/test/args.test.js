...
const RESNET = { numClasses: { kind: "int", min: 1 } };
function rejects(given, mentions) {
    assert.throws(() => checkArgs("resnet18", RESNET, given), (err) => {
...
test("거절 메시지가 받는 인자 목록을 같이 말한다", () => {
    ...
    assert.throws(() => checkArgs("resnet18", RESNET, { classes: 10 }), (err) => err instanceof BimmError && err.message.includes("numClasses"));
});
